...
module.exports = class IasTemplate extends require('../../plugin') {
    ...
    async run() {
        ...
        if (project.hasMultitenancy) {
            await merge(__dirname, 'files/package.json.hbs').into('mtx/sidecar/package.json', { project, forceOverwrite: true })
        }
    ...
    async combine() {
        ...
        if (hasMta) {
            ...
            const additions = [srv, identity, srvIdentityBinding]
            if (hasApprouter) {
                const approuterIdentityBinding = { in: [approuter, 'requires'], where: { 'parameters.config.app-identifier': 'approuter' } }
                additions.push(approuter, approuterIdentityBinding)
            }
            if (hasMultitenancy) {
            ...
            await merge(__dirname, 'files/mta.yaml.hbs').into('mta.yaml', { project, additions })
        }
    ...
    async #combineWithCfManifest(project) {
        ...
        const identityBinding = { in: 'services', where: { 'parameters.app-identifier': project.appName } }
