...
{{/*
Define Hosts for the APIRule (since v2alpha1)
*/}}
{{- define "web-application.internal.exposeHosts" -}}
{{- if .Values.expose.hosts }}
{{ .Values.expose.hosts | toYaml }}
{{- else }}
{{- $name := (include "web-application.fullname" .) }}
{{- if hasPrefix $name .Release.Namespace }}
- {{ .Release.Namespace }}
...
{{- if $ips.local.name }}
{{- $ips.local.name }}
{{- else if or (and $ips.local $ips.local.dockerconfigjson) (and $ips.global $ips.global.dockerconfigjson) }}
{{- include "web-application.fullname" . }}
{{- else if and $ips.global $ips.global.name }}
{{- $ips.global.name }}
{{- end }}
{{- end }}
...
{{- define "web-application.internal.healthCheck" }}
{{ .checkName }}:
{{- if .checkValues.path }}
  httpGet:
    path: {{ .checkValues.path }}
    port: http
{{- else }}
  tcpSocket:
    port: {{ .context.Values.port }}
{{- end }}
{{- if eq .checkName "startupProbe" }}
  periodSeconds: 2
  failureThreshold: {{ div (add1  .context.Values.startupTimeoutSeconds) 2 }}
{{- else }}
{{- if .checkValues.periodSeconds }}
  periodSeconds: {{ .checkValues.periodSeconds }}
{{- end }}
{{- if .checkValues.failureThreshold }}
  failureThreshold: {{ .checkValues.failureThreshold }}
{{- end }}
{{- if .checkValues.successThreshold }}
  successThreshold: {{ .checkValues.successThreshold }}
{{- end }}
{{- if .checkValues.timeoutSeconds }}
  timeoutSeconds: {{ .checkValues.timeoutSeconds }}
{{- end }}
{{- end }}
{{- end }}
