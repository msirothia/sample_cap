...
let commonOptions = ['no_copy', 'no_save', 'out', 'dry', 'as', 'from', 'force', 'config', 'destination', 'group', 'name'];
...
async function _write(result, service, cwd) {
    let dest, output, using, message, extension;
    output = result[0];
    dest = result[1];
    await write(dest, output, "utf-8");
    extension = dest.substring(dest.lastIndexOf('.'));
    dest = dest.replace(extension, '');
     if (service.length > 1){
...
async function _updatePackageJson(dest, services, kind, cwd, config, destination) {
    // try to find package.json in the srv folder
    let package_json = _findPackageJson(path.resolve(cwd, dest), cwd);
    let conf = {};
    if (package_json) {
        conf = JSON.parse(await read(package_json, "utf-8"));
    } else {
    ...
    if (!config && destination) {
        config = { "[production]": { credentials: { destination } } };
    }
    ...
    for (let service of services) {
        ...
        const serviceConfig = { kind, model, ...config }
        if (!requires[service]) {
...
async function postProcess(filePath, options, csn, cwd) {
    ...
    const destFilePath = path.resolve(cwd, options.out ? path.join(options.out, path.basename(filePath)) : path.join(cds.env.folders.srv, 'external', path.basename(filePath)));
    ...
    const registered = options.dry || options.no_save || await _updatePackageJson(result[1].replace(/\.[^.]+$/, ''), services, kind, cwd, config, options.destination);
