const watchOnlyOptions = ['--ext', '--livereload', '--open', '--debug', '--inspect', '--inspect-brk']
...
const DEBUG = cds.log('cli|watch')._debug
...
const ignore = RegExp(
  `(\\.git|node_modules|_out|@types|@cds-models)\\${sep}` +
  `|app(\\${sep}.+)?\\${sep}(` +
...
async function watch ([cwd], {
  ...
  const watcher = nodemon ({
    script: resolve(__dirname,'../lib/watch/watched'), cwd, env, ext, ignore,
    delay,
    options
  }).on('restart', (files)=>{
