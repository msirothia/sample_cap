      ...
      if (task) {
        ...
        if (cmd === 'add') {
          ...
          if (argv.includes('-p') || argv.includes('--package')) {
            const p = argv[argv.indexOf('-p') + 1] || argv[argv.indexOf('--package') + 1]
            console.log(`running with --package, adding dependency ${p}`)
            await require('../lib/util/command').spawnCommand(`npm`, ['add', p], { cwd: cds.root })
            delete require.cache[require.resolve(path.join(cds.root,'./package.json'))]
          }
          cds.env.plugins = cds.env.for('cds').plugins // reload plugin env
          cds.add = require('../lib/init/add')
          await cds.plugins
  ...
  if (cds.cli?.command === 'serve' && fs.existsSync(tsconfig)) { // for `cds serve` started directly or through watch
    ...
    if (!process.env.CDS_TYPESCRIPT ) {
      try {
        ...
        console.log(`Detected ${path.relative(cds.root, tsconfig)}. Running with tsx.\n`)
        process.env.CDS_TYPESCRIPT = 'tsx'
      }
      catch (e) {
