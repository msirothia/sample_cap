...
module.exports = {
  ...
  sendStateParameter: async function ({method, state, req}) {
    ...
    const reqOptions = {
      ...
      timeout: module.exports.STATE_PARAMETER_TIMEOUT,
    ...
    const responseMessage = (response.response && response.response.data) || response.message;
    if (response.statusCode === 200){
      return;
    } else if (response.statusCode === 401){
      const loggingData = logger.getAuditLogAdditionalData(req);
      logger.writeToAuditLog(req, loggingData, `${responseMessage} - Source: ${reqOptions.url}`,  (err) => {
        if (err) {
          req.loggingContext.error('Error occurred when writing to audit log. Error: %s', err.message);
        }
      });
    }
    const err = new Error(`${responseMessage} - Source: ${reqOptions.url}`);
